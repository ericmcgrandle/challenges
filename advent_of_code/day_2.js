...
const validatePassword = (passwords) => {
  ...
  passwords.map((password) => {
    const min = password[0].match(/^\d+/);
    const max = password[0].match(/[^-]*$/);

    const char = new RegExp(password[1], 'g');

    const numOfCharsInString = password[2].match((char) || [].length);

    if (numOfCharsInString === null) {
      return false;
    } else if (numOfCharsInString.length >= min && numOfCharsInString.length <= max) {
      count++;
      return true;
    } 
  });
