// Challenge taken from https://adventofcode.com/2020/day/2

// Passwords are in the format of :
// firstIndex-secondIndex, character, password
const validatePassword = (passwords) => {

  let count = 0;

  passwords.map((password) => {
    const firstIndex = password[0].match(/^\d+/);
    const secondIndex = password[0].match(/[^-]*$/);

    const actualPassword = password[2];

    if (actualPassword[firstIndex[0] - 1] === password[1] && actualPassword[secondIndex[0] - 1] !== password[1]) {
      count++;
    } else if (actualPassword[firstIndex[0] - 1] !== password[1] && actualPassword[secondIndex[0] - 1] === password[1]) {
      count++;
    }
...
// Call validatePassword under passwords
